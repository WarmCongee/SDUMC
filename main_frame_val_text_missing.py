    ...
    for ii in range(len(train_loaders)):
        ...
        for epoch in range(args.epochs):
            ...
            if test_result_full['mae'] <= best_epoch_test_full['mae']: # and test_result['f1'] >= best_epoch_test['f1']:
                ...
                print("***************better full**********************")
                if os.path.exists(last_sava_path_full):
                    os.remove(last_sava_path_full) 
                ...
                last_sava_path_full = f'mosei_mult-view_kd_full_{best_epoch_test_full["mae"]}_{epoch+1}.pt'
            ...
            if test_result_missing['mae'] <= best_epoch_test_missing['mae']: # and test_result['f1'] >= best_epoch_test['f1']:
                ...
                print("===============better missing===================")
                if os.path.exists(last_sava_path):
                    os.remove(last_sava_path) 
                # torch.save({'epoch': epoch+1, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}, f'mosei_mult-view_kd_missing_{best_epoch_test_missing["mae"]}_{epoch+1}.pt')
                last_sava_path = f'mosei_mult-view_kd_missing_{best_epoch_test_missing["mae"]}_{epoch+1}.pt'
