...
import sys
sys.path.append('../../')
import SpeakerInvariantMER.config_backup as config_backup
from dataset import MixupFaceDataset
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mix_dir', type=str, default='UTTERANCE', help='feature level [FRAME or UTTERANCE]')
    parser.add_argument('--gpu', type=str, default='1', help='gpu id')
    params = parser.parse_args()
    os.environ["CUDA_VISIBLE_DEVICES"] = params.gpu

    print(f'==> Extracting manet embedding...')
    face_dir = '/disk3/htwang/MER2023-Baseline-master/dataset-process/features-train/openface_face'
    save_dir = os.path.join('/disk3/htwang/MER2023-Baseline-master/dataset-process', params.mix_dir, f'manet_{params.feature_level[:3]}')
    if not os.path.exists(save_dir): os.makedirs(save_dir)
    
    with codecs.open(os.path.join('/disk3/htwang/MER2023-Baseline-master/dataset-process', params.mix_dir, 'name2sample_weight.json'), 'r') as handle:
        mix_dic = json.load(handle)

    # load model
    model = manet(num_classes=7).cuda()
    checkpoint_file = os.path.join(config_backup.PATH_TO_PRETRAINED_MODELS, 'manet/[02-08]-[21-19]-model_best-acc88.33.pth')
    ...
    for i, vid in enumerate(vids, 1):
        ...
        # forward
        dataset = MixupFaceDataset(vid, face_dir, mix_dic, transform=transform)
        if len(dataset) == 0:
        ...
        if params.feature_level == 'FRAME':
            ...
            np.save(csv_file, embeddings)
# python -u extract_manet_embedding_mixup.py --dataset=MER2023 --feature_level='UTTERANCE' --gpu=3 --mix_dir=mixup_202307041119
