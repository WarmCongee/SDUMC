...
VICUNA_7B  = 'vicuna-7b-v1.5' # https://huggingface.co/lmsys/vicuna-7b-delta-v0
...
BAICHUAN2_13B_4bits = 'Baichuan2-13B-Chat-4bits' # https://huggingface.co/baichuan-inc/Baichuan2-13B-Chat-4bits
...
def extract_embedding(model_name, trans_dir, save_dir, feature_level, gpu=-1, punc_case=None, language='chinese', model_dir=None):
    ...
    if punc_case is None and language == 'chinese' and model_dir is None:
        save_dir = os.path.join(save_dir, f'{model_name}-{feature_level[:3]}-base-whisper')
    elif punc_case is not None:
        save_dir = os.path.join(save_dir, f'{model_name}-punc{punc_case}-{feature_level[:3]}-base-whisper')
    elif language == 'english':
        save_dir = os.path.join(save_dir, f'{model_name}-langeng-{feature_level[:3]}-large-v2-whisper')
    elif model_dir is not None:
        prefix_name = "-".join(model_dir.split('/')[-2:])
        save_dir = os.path.join(save_dir, f'{prefix_name}-{model_name}-{feature_level[:3]}-base-whisper')
    ...
    elif model_name in [BAICHUAN_7B, BAICHUAN_13B, BAICHUAN2_7B, BAICHUAN2_13B, BAICHUAN2_13B_4bits]:
        model = AutoModelForCausalLM.from_pretrained(model_dir, low_cpu_mem_usage=True, trust_remote_code=True, revision='main')
        tokenizer = AutoTokenizer.from_pretrained(model_dir, use_fast=False, trust_remote_code=True)
    ...
    if torch.cuda.device_count() > 1:
        ...
        device_map = infer_auto_device_map(model, max_memory=get_balanced_memory(model), no_split_module_classes=["layers"])
        print(device_map)
    ...
    for idx, row in df.iterrows():
        ...
        if pd.isna(sentence) == False and len(sentence) > 0:
            ...
            inputs_generat = tokenizer(sentence_raw, return_tensors='pt')

            if gpu != -1: inputs_generat = inputs_generat.to('cuda')
            with torch.no_grad():

                outputs_all = model(**inputs_generat, output_hidden_states=True)
                # print(outputs.keys())
                # print(outputs_prompt.logits.shape)
                # print(outputs_raw.logits.shape)
                # print(outputs_all.logits.shape)
                # print(outputs_raw.hidden_states[0].shape)
                # returen_sentence = model.generate(**inputs, max_new_tokens=64,repetition_penalty=1.1)
                # print(returen_sentence.shape)
                # # print(returen_sentence.keys())
                # print(tokenizer.decode(returen_sentence.cpu()[0], skip_special_tokens=True))
                outputs = outputs_all.hidden_states
...
if __name__ == '__main__':
    ...
    trans_dir = '/disk6/yzwen/SpeakerInvariantMER/dataset/mosei_text_asr_whisper-large-v2_vad.csv'
